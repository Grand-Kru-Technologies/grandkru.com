<template>
  <ul class="inline-link-list">
    ...
    <li v-for="(link, index) in parsedLinks" :key="index" class="inline-link-list-item">
      <a
      ...
      >
        <i v-if="link.faIcon" :class="link.faIcon" class="inline-link-list-icon" />
        {{ link.label }}
      </a>

      <router-link v-else :to="link.href" class="text-2 inline-link-list-link" :class="link.classList">
        <i v-if="link.faIcon" :class="link.faIcon" class="inline-link-list-icon" />
        {{ link.label }}
      </router-link>
    </li>
...
const parsedLinks = computed(() => {
  return props.items.map((item) => ({
    ...
    isExternalLink: (item.href || "").charAt(0) !== "/",
    classList: {
